...
test_that("buzzEBMcontMcontY runs end-to-end without crashing", {
  set.seed(123)
  n <- 30
  toy_data <- data.frame(
    X = rnorm(n),
...
test_that("buzzEBMcontMcontY catches bad formulas before running JAGS", {

  toy_data <- data.frame(X = 1:10, M1 = 1:10, Y = 1:10)

  # Assert: It should immediately fail using the parser we tested earlier
  expect_error(
    buzzEBMcontMcontY(model = "Y = M1 | X", dataset = toy_data),
