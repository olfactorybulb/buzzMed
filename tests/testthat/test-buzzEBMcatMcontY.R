...
test_that("buzzEBMcatMcontY runs end-to-end without crashing", {
  set.seed(123)
  n <- 30
  toy_data <- data.frame(
    X = rnorm(n),               # X can be anything, usually continuous
    M1 = rbinom(n, 1, 0.3),     # M is Binary
    M2 = rbinom(n, 1, 0.7),     # M is Binary
    Y = rnorm(n)                # Y is Continuous
  )

  res <- buzzEBMcatMcontY(
    model    = "Y ~ M1 + M2 | X",
    dataset  = toy_data,
    n_chains = 1,
    n_burnin = 10,
...
test_that("buzzEBMcatMcontY catches bad formulas before running JAGS", {

  toy_data <- data.frame(X = 1:10, M1 = 1:10, Y = 1:10)

  # Assert: It should immediately fail using the parser we tested earlier
  expect_error(
    buzzEBMcatMcontY(model = "Y = M1 | X", dataset = toy_data),
